...
void GameEngine::Initialise()
{
	...
	// Setup up the device.
	//cudaStatus = cudaClass.CudaSetup
...
void GameEngine::UpdateGame(void)
{
	...
	if (start) {
		...
		if (elapsedTime.count() >= waitForTime) {
			...
			for (int i = 0; i < gnomes.size(); i++) // replace with the movement function for the gnome
			{
...
void GameEngine::Setup()
{
	...
	///// Create fungi ////////////
	#pragma region fungus Instances:)
